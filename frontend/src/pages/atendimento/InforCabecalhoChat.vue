...
export default {
  ...
  computed: {
    ...
    cSessions() {
      return this.whatsapps.filter(w => ["whatsapp", "baileys", "meow", "waba"].includes(w.type) && !w.isDeleted && w.status === 'CONNECTED');
    },
    cSessionsOptions() {
  ...
  methods: {
    ...
    async confirmarTransferenciaCanal () {
      ...
      try{
        ...
        this.$q.notify({
          type: 'info',
          message: 'Ticket transferido para o canal',
          progress: true,
          actions: [{
            icon: 'close',
            round: true,
        ...
        if (e.status === 409) {
          this.$q.notify({
            type: 'warning',
            message: 'Já existe um ticket aberto/pendente para esse contato. ',
            progress: true,
            actions: [{
              icon: 'close',
              round: true,
