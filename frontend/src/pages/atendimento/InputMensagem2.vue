...
export default defineComponent({
  ...
  data() {
    return {
      ...
      gptEnabled: false,
      loading: false,
      accept: '.exe, .ai, .eps, .csv, .rar, .kml, .psd, .txt, .xml, .jpg, .png, .pdf, .doc, .docx, .mp4, .xls, .xlsx, .jpeg, .zip, .ppt, .ogg, .mp3, .pptx, image/*, .mpeg, .pfx, .p2k',
  ...
  methods: {
    ...
    handleFileDrop(event) { //Ruan Alteração: função alterada para pegar vários arquivos
      ...
      if (files.length) {
        ...
        for (let i = 0; i < files.length; i++) {
          const file = files[i];
          this.arquivos.push(file);  // Armazena todos os arquivos arrastados
        }

        this.abrirModalPreviewImagem = false;

        // Atualiza o objeto urlMediaPreview para incluir nomes e URLs
        this.urlMediaPreview = {
          title: `Enviar ${this.arquivos.length} imagens para ${this.ticketFocado?.contact?.name}`,
          files: this.arquivos.map((file) => ({
            name: file.name,  // Nome do arquivo
            src: this.openFilePreviewDD(file)  // URL para pré-visualização
          })),
        };
