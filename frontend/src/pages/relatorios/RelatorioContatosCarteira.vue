<template>
  <div v-if="userProfile === 'admin' || userProfile === 'super' ">
    <q-card bordered>
      <q-card-section>
        <div class="text-h6 q-px-sm"> Relatório de Contatos por Carteiras </div>
      </q-card-section>
  ...
  methods: {
    ...
    async gerarRelatorio () {
      if (!this.pesquisa.wallets.length) {
        this.$notificarErro('Ops... Para gerar o relatório, é necessário selecionar pelo menos uma carteira.')
        return
      }
